<template>
  ...
  <v-row class="d-flex">
    ...
    <v-col>
      <v-btn height="40" variant="outlined" @click="calculateDate">
        Рассчитать дату
      </v-btn>
    </v-col>
  </v-row>
...
export default class Square extends Vue {
  ...
  get items(): Dates {
    return {
      ...
      years: new Array(60)
        .fill(1)
        .map((e, i) => i + (new Date().getFullYear() - 59)),
    };
  }
  dlList = [
    {
  ...
  calculateDate(): void {
    ...
    const indexMonth = this.items.months.findIndex(
      (month) => month === this.date.month
    );
    ...
    const twoNumber = String(oneNumber)
      .split("")
      .reduce((a, b) => Number(a) + Number(b), 0);
    const threeNumber = oneNumber - Number(String(oneStringPF)[0]) * 2;
    const fourNumber = String(threeNumber)
      .split("")
      .reduce((a, b) => Number(a) + Number(b), 0);
    const twoStringPF = `${oneNumber}${twoNumber}${threeNumber}${fourNumber}`
      .split("")
      .map((i) => Number(i));
